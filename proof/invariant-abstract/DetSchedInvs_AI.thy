...
abbreviation tcb_of2 :: "Structures_A.kernel_object \<rightharpoonup> Structures_A.tcb" where
  "tcb_of2 ko \<equiv> case ko of kernel_object.TCB tcb \<Rightarrow> Some tcb | _ \<Rightarrow> None"
...
abbreviation
  "tcb_scps_of2 s \<equiv> tcbs_of2 s ||> tcb_sched_context"

lemma tcb_scps_of_rewrite:
  "tcb_scps_of s tp = tcb_scps_of2 s tp"
  by (fastforce simp: vs_all_heap_simps opt_map_def
              split: option.splits Structures_A.kernel_object.splits)
...
lemma sc_at_pred_rewrite:
  ...
  by (clarsimp simp: opt_map_def sc_at_pred_def obj_at_def
              split: option.splits kernel_object.splits)
 
lemma sc_at_ppred_rewrite:
  "sc_at_ppred proj P scp s = (P |< scs_of2 s ||> proj) scp"
  by (clarsimp simp: opt_map_def sc_at_ppred_def obj_at_def
              split: option.splits kernel_object.splits)
...
abbreviation ep_of2 :: "kernel_object \<rightharpoonup> endpoint" where
  "ep_of2 ko \<equiv> case ko of Endpoint endpoint \<Rightarrow> Some endpoint | _ \<Rightarrow> None"
...
abbreviation eps_of2 :: "'z state \<Rightarrow> obj_ref \<rightharpoonup> endpoint" where
  "eps_of2 s \<equiv> kheap s |> ep_of"

abbreviation "ep_send_qs_of2 s \<equiv> eps_of2 s |> send_q_of2"
abbreviation "ep_recv_qs_of2 s \<equiv> eps_of2 s |> recv_q_of2"

lemma eps_of_rewrite: "eps_of s ep = eps_of2 s ep" by (clarsimp simp: eps_of_def)
...
lemma is_refill_sufficient_rewrite:
  ...
  by (fastforce simp: is_refill_sufficient2_def vs_all_heap_simps active_sc_def opt_map_left_Some
               split: option.split_asm Structures_A.kernel_object.splits)
...
abbreviation active_sc_tcb_at2 where
  "active_sc_tcb_at2 tp s \<equiv> active_sc_tcb_at_pred2 (tcb_scps_of2 s) (scs_of2 s) tp"
...
lemma active_sc_tcb_at_rewrite:
  ...
  by (fastforce simp: opt_flatten_def vs_all_heap_simps is_sc_obj map_join_simps pred_map_rewrite
                      obj_at_def active_sc_def opt_map_def
               split: option.splits Structures_A.kernel_object.splits)

definition budget_ready2 where
  "budget_ready2 tp s \<equiv> ((\<lambda>scp. is_refill_ready2 scp s) |< (opt_flatten o tcb_scps_of2 s)) tp"
...
definition tcb_refills_neq_Nil where
  "tcb_refills_neq_Nil tp s \<equiv> ((\<lambda>sc. sc_refills sc \<noteq> []) |<
                                        (opt_flatten o tcb_scps_of2 s) |> scs_of2 s) tp"

lemma budget_ready_rewrite:
  "budget_ready tp s = budget_ready2 tp s"
  by (rule iffI;
      fastforce simp: opt_flatten_def vs_all_heap_simps opt_map_left_Some hd_opt_def
                      is_refill_ready2_def refill_ready'_def budget_ready2_def
               split: option.splits kernel_object.splits)

definition budget_sufficient2 where
  "budget_sufficient2 tp s \<equiv> ((\<lambda>scp. is_refill_sufficient2 0 scp s) |< (opt_flatten o tcb_scps_of2 s)) tp"
...
abbreviation released_sc_tcb_at2 where
  "released_sc_tcb_at2 tp s \<equiv> released_sc_tcb_at_pred2 (cur_time s) (tcb_scps_of2 s) (scs_of2 s) tp"
...
lemma released_sc_tcb_at2_def2:
  ...
  by (fastforce simp: budget_ready2_def budget_sufficient2_def opt_map_left_Some opt_flatten_def
...
abbreviation released_sc_at2 :: "obj_ref \<Rightarrow> 'z state \<Rightarrow> bool" where
...
lemma released_sc_at_rewrite:
  ...
  by (fastforce simp: pred_map_rewrite vs_all_heap_simps opt_map_left_Some active_sc_def
               split: option.splits kernel_object.splits)

definition bound_sc_obj_tcb_at2 where
  "bound_sc_obj_tcb_at2 P tp s \<equiv> (P |< (opt_flatten o tcb_scps_of2 s) |> scs_of2 s) tp"

lemma bound_sc_obj_tcb_at_rewrite:
  "bound_sc_obj_tcb_at P tp s = bound_sc_obj_tcb_at2 (P o sc_refill_cfg_of) tp s"
  by (fastforce simp: opt_flatten_def vs_all_heap_simps opt_map_left_Some bound_sc_obj_tcb_at2_def
                 split: option.splits kernel_object.split_asm)

abbreviation st_tcb_at2 where
  "st_tcb_at2 P tp s \<equiv> (P |< tcbs_of2 s ||> tcb_state) tp"

lemma st_tcb_at_rewrite:
  "st_tcb_at P tp s = st_tcb_at2 P tp s"
  by (fastforce simp: pred_tcb_at_def obj_at_def opt_map_left_Some
                 split: option.splits kernel_object.split_asm)
...
lemma is_schedulable_bool_rewrite:
  ...
  by (clarsimp simp: is_schedulable_bool_def is_schedulable_bool2_def
                     is_sc_active_rewrite is_active_sc2_def get_tcb_def
              split: option.splits kernel_object.splits)
     (fastforce simp: opt_map_def opt_flatten_def split: option.splits)+
...
abbreviation "tcb_ready_times_of2 s \<equiv> opt_flatten o tcb_scps_of2 s |> scs_of2 s ||> sc_ready_time2"
...
abbreviation valid_ready_qs2 :: "'z state \<Rightarrow> bool" where
  "valid_ready_qs2 s \<equiv> valid_ready_qs2_2 (ready_queues s) (cur_time s) (etcbs_of s) (tcb_sts_of s) (tcb_scps_of s) (scs_of2 s)"
...
lemma bounded_release_time_rewrite:
  ...
  by (fastforce simp: pred_map_rewrite opt_map_left_Some vs_all_heap_simps
               split: option.splits kernel_object.splits)

definition active_sc_valid_refills2_2 :: "time \<Rightarrow> (obj_ref \<rightharpoonup> sched_context) \<Rightarrow> bool" where
...
abbreviation active_sc_valid_refills2 :: "'z state \<Rightarrow> bool" where
...
lemma active_sc_valid_refills_rewrite:
  ...
  by (fastforce simp: active_sc_valid_refills2_2_def active_sc_valid_refills_2_def is_active_sc2_def
                      opt_map_def is_active_sc_rewrite
...
abbreviation sorted_release_q2 :: "'z state \<Rightarrow> bool" where
  "sorted_release_q2 s \<equiv> sorted_release_q2_2 (opt_flatten o tcb_scps_of2 s |> scs_of2 s) (release_queue s)"
...
abbreviation valid_release_q_except_set2 :: "obj_ref set \<Rightarrow> 'z state \<Rightarrow> bool" where
  "valid_release_q_except_set2 except s \<equiv>
      valid_release_q_except_set2_2 except (release_queue s) (tcb_sts_of2 s) (tcb_scps_of2 s) (scs_of2 s)"
...
lemma valid_release_q_except_set_2_rewrite:
  ...
  by (simp add: pred_map_rewrite)

abbreviation "valid_release_q2_2 \<equiv> valid_release_q_except_set2_2 {}"
abbreviation "valid_release_q2 s \<equiv> valid_release_q2_2 (release_queue s) (tcb_sts_of2 s) (tcb_scps_of2 s) (scs_of2 s)"
...
abbreviation released_if_bound_sc_tcb_at2 :: "obj_ref \<Rightarrow> 'z state \<Rightarrow> bool" where
  "released_if_bound_sc_tcb_at2 t s \<equiv> released_if_bound_sc_tcb_at2_2 t (cur_time s) (tcb_scps_of2 s) (scs_of2 s)"

lemma released_if_bound_sc_tcb_at_rewrite:
  "released_if_bound_sc_tcb_at tp s = released_if_bound_sc_tcb_at2 tp s"
  by (clarsimp simp: released_sc_tcb_at_rewrite tcb_scps_of_rewrite)
...
abbreviation active_if_bound_sc_tcb_at2 :: "obj_ref \<Rightarrow> 'z::state_ext state \<Rightarrow> bool" where
  "active_if_bound_sc_tcb_at2 t s \<equiv> active_if_bound_sc_tcb_at2_2 t (tcb_scps_of2 s) (scs_of2 s)"

lemma active_if_bound_sc_tcb_at_rewrite:
  "active_if_bound_sc_tcb_at tp s = active_if_bound_sc_tcb_at2 tp s"
  by (clarsimp simp: active_sc_tcb_at_rewrite tcb_scps_of_rewrite)
...
abbreviation valid_sender_sc_tcb_at2 :: "obj_ref \<Rightarrow> 'z state \<Rightarrow> bool" where
  "valid_sender_sc_tcb_at2 t s \<equiv> valid_sender_sc_tcb_at2_2 t (cur_time s) (tcb_scps_of2 s) (tcb_faults_of2 s) (scs_of2 s)"
...
lemma valid_sender_sc_tcb_at_rewrite:
  ...
  unfolding active_sc_tcb_at_rewrite released_sc_tcb_at_rewrite tcb_scps_of_rewrite
            timeout_faulted_tcb_at_rewrite
  by simp
...
abbreviation released_ipc_queues_pred2 where
  "released_ipc_queues_pred2 P \<equiv>
    \<lambda>s. P (cur_time s) (tcb_sts_of2 s) (tcb_scps_of2 s) (tcb_faults_of2 s) (scs_of2 s)"
...
lemmas projection_rewrites = pred_map_rewrite scs_of_rewrite is_active_sc_rewrite
                             opt_map_left_Some sc_heap_of_state_def sc_of_def sc_refills_sc_at_rewrite
                             valid_refills_rewrite round_robin_rewrite
                             is_active_sc_rewrite[simplified pred_map_rewrite scs_of_rewrite]
